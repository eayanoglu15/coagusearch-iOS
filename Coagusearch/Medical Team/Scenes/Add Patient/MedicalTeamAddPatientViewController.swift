//
//  MedicalTeamAddPatientViewController.swift
//  Coagusearch
//
...
extension MedicalTeamAddPatientViewController: MedicalTeamAddPatientDataSourceDelegate {
    func showPatientLoginInfo(id: Int, code: String) {
        let title = "Patient Login Info".localized
        let message = "This patient can use the system with following informations".localized + "\n" + "ID: " + "\(id)" + "\n" + "Password: ".localized + "\(code)"
...
class MedicalTeamAddPatientViewController: BaseScrollViewController {
    var dataSource = MedicalTeamAddPatientDataSource()
    
    @IBOutlet weak var infoView: UIView!
    
    @IBOutlet weak var idLabel: UILabel!
    @IBOutlet weak var idTextField: UITextField!
    @IBOutlet weak var idLabelTopConstraint: NSLayoutConstraint!
    ...
    override func viewDidLoad() {
        ...
        if let newPatientId = dataSource.getPatientId() {
            idTextField.text = "\(newPatientId)"
            floatTitle(textField: idTextField)
        }
        
        stylize()
        title = "Add Patient".localized
        // Do any additional setup after loading the view.
        idTextField.delegate = self
        idLabel.textColor = .dodgerBlue
        idTextField.bottomBorderColor = UIColor.lightBlueGrey.withAlphaComponent(0.5)
    ...
    @IBAction func saveButtonTapped(_ sender: Any) {
        guard let idStr = idTextField.text, !idStr.isEmpty, let id = Int(idStr) else {
            showAlertMessage(title: "Missing Information".localized, message: "Enter patient identity number".localized)
            return
        }
        guard let name = nameTextField.text, !name.isEmpty, let surname = surnameTextField.text, !surname.isEmpty else {
            showAlertMessage(title: "Missing Information".localized, message: "Enter patient name and surname".localized)
            return
        }
        var birthDay: Int?
        var birthMonth: Int?
        ...
        dataSource.addPatient(id: id, name: name, surname: surname, birthDay: birthDay, birthMonth: birthMonth, birthYear: birthYear, height: height, weight: weight, bloodType: bloodType, rhType: rhType, gender: gender)
    }
...
extension MedicalTeamAddPatientViewController: UITextFieldDelegate {
    ...
    func floatTitle(textField: UITextField) {
        if textField == idTextField {
            idLabel.font = fontRegular
            idLabelTopConstraint.constant = -30
            idLabel.textColor = .dodgerBlue
            idTextField.bottomBorderColor = UIColor.dodgerBlue.withAlphaComponent(0.5)
        }
    ...
    private func performAnimation(textField: UITextField, transform: CGAffineTransform) {
        if textField == idTextField {
            UIView.animate(withDuration: 0.2, delay: 0, options: .curveEaseOut, animations: {
                self.idLabel.transform = transform
    ...
    func unfloatTitle(textField: UITextField) {
        if textField == idTextField {
            ...
            idLabelTopConstraint.constant = 0
            idLabel.font = fontLight
            idTextField.bottomBorderColor = UIColor.lightBlueGrey.withAlphaComponent(0.5)
        }
        if textField == nameTextField {
